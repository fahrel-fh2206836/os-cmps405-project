...
CREATE USER IF NOT EXISTS 'dev_lead1'@'%' IDENTIFIED BY 'Dev1234@';
...
#sudo bash -c "cat > /etc/systemd/system/mysql_log_dev.service" << EOF
#[Unit]
#Description=MySQL Log Monitoring Service
#After=network.target
#[Service]
#ExecStart=/bin/bash /path/to/your/script/MySQL_login_dev_lead1.sh (Depends on where you save the script)
#Restart=always
#User=root
#[Install]
