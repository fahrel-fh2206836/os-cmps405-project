...
CREATE USER IF NOT EXISTS 'ops_lead1'@'%' IDENTIFIED BY 'Ops1234@';
...
#sudo bash -c "cat > /etc/systemd/system/mysql_log_ops.service" << EOF
#[Unit]
#Description=MySQL Log Monitoring Service
#After=network.target
#[Service]
#ExecStart=/bin/bash /path/to/your/script/MySQL_login_ops_lead1.sh (Depends on where you save the script)
#Restart=always
#User=root
#[Install]
...
#sudo systemctl enable mysql_log_ops.service;
#sudo systemctl start mysql_log_ops.service;